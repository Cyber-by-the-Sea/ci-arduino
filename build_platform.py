...
def test_examples_in_folder(folderpath):
    ...
    for example in sorted(os.listdir(folderpath)):
        ...
        if BUILD_WARN:
            if os.path.exists(gen_file_name):
                cmd = ['arduino-cli', 'compile', '--build-property', 'build.extra_flags=-Wall', '--warnings', 'all', '--fqbn', fqbn, '-e', folderpath]
            else:
                cmd = ['arduino-cli', 'compile', '--build-property', 'build.extra_flags=-Wall', '--warnings', 'all', '--fqbn', fqbn, folderpath]
        else:
...
def test_examples_in_learningrepo(folderpath):
    ...
    for project in os.listdir(folderpath):
        ...
        cmd = ['arduino-cli', 'compile', '--build-property', 'build.extra_flags=-Wall', '--warnings', 'all', '--fqbn', fqbn, projectpath]
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        r = proc.wait()
        out = proc.stdout.read()
